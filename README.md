...
A personal, local-first fantasy football draft-day dashboard. M6.3 builds a persistent SQLite database from real NFL teams, active Sleeper QB/RB/WR/TE players, 2025 nflverse weekly statistics, and 2026 FantasyPros Aggregate ADP plus Draft ECR/tiers.
...
Once both caches exist, database builds and loads never contact FantasyPros.
...
The Sleeper player response is cached under `./data/import-cache/`. Re-running within 24 hours uses that cache instead of repeating Sleeper's large player request. The nflverse stats and DynastyProcess ID crosswalk are also cached there and reused indefinitely because they describe a completed historical season. Aggregate ADP and Draft ECR have independent response/metadata caches. These directories and SQLite files are gitignored.
...
All loaders are idempotent. `build` currently runs teams, players, stats, then FantasyPros data. `load stats` uses the validated local cache when present; add `--refresh` only when you deliberately want to download and validate both public provider files again. `load fantasypros`, `build`, and `rebuild` require both FantasyPros caches and cannot make a provider request. Each refresh caches one response; `load fantasypros` then validates and replaces 2026 Aggregate ADP, ECR, and tiers together in one transaction. Later M6.x milestones will add odds to the same entry point.
...
After the two explicit refreshes and `load fantasypros` (or a confirmed rebuild), Overview and Draft Day show Aggregate ADP, overall ECR, position rank, overall tier, and the ECR minimum/maximum/standard deviation. Missing or unmatched values remain `—` rather than zero.

Odds and live picks are deliberately not populated yet. Those UI fields render `—`, and all players remain available. Odds and the final real-data audit will be implemented in M6.4–M6.5. Live Sleeper draft synchronization begins in M7.
