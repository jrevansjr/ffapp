...
**Available-player table.** Columns: Name, Position, NFL team, Sleeper depth-chart position/order, FantasyPros overall tier, overall ECR, position rank, and Aggregate ADP. The combined Depth column displays values such as `SWR 2` and sorts by numeric order, then depth position, with missing values last. Default sort: Aggregate ADP ascending, missing ADP last. Filters: position, NFL team, tier; a search box only if simple. Taken players are **removed** from this table, updating as the draft-state query receives new picks — no full page refresh, ever.
...
- **Injury context:** a compact Sleeper-sourced status panel with designation, body part, optional notes, practice participation, injury start date, and player-data refresh time. It reports provider facts without predicting availability or recovery.
...
- **2025 performance:** weekly 0.5-PPR average, high, median, and low grouped with the distinct NFL team or teams represented in nflverse weekly rows, games played, targets/game, and carries/game above the charts. Median computed correctly. No weekly data → empty state, not fake zeros.
...
- **2026 Sportsbook Consensus:** a separate position-aware card. QB shows passing and rushing yards/TDs; RB shows rushing and receiving yards/TDs; WR/TE show receiving yards/TDs. Every player with an NFL team also shows that team's projected regular-season wins. Missing lines remain `—`; this card never replaces FantasyPros projections.
...
`/api/players` returns the fields Overview and Draft Day's table need (name, position, team, years experience, Aggregate ADP, ECR/position rank/expert range, tier, 2025 season summary fields, 2026 FantasyPros projections, and `is_taken`). `/api/players/{id}` additionally returns the explicit player-futures markets and team win total used by the inspector. These are presentation fields — they do not imply denormalized columns. Both endpoints read persisted SQLite data only and never call a provider; imports and draft-pick polling are separate operations.
...
- **`player_week_stats`** — key `(player_id, season, week)`: same stat fields plus nullable `nfl_team_id` for the team represented in that historical week. The approved stats importer calculates half-PPR points with one small explicit function from raw weekly stats.
...
- **`odds`** — one generic table: `id, season, source, market, player_id (nullable), nfl_team_id (nullable), line, over_price (nullable), under_price (nullable), captured_at`. Exactly one of player/team identifies the subject. M8.1 stores the six passing/rushing/receiving yard/TD markets plus `regular_season_wins` as source `sportsbook_consensus`. Consensus prices remain null because the supplied snapshot provides a line consensus, not a consensus price.
- **`player_morality_scores`** — key `(player_id, source)`: an integer 0–5 subjective score, snapshot date, and UTC import timestamp. M8.2 stores the manually supplied source-dated snapshot as `user_supplied`; it is not a player property, provider fact, or app-generated recommendation.
...
- 2025 weekly/season stats: approved nflverse weekly CSV, joined by GSIS ID. Missing local GSIS IDs may be backfilled only by exact Sleeper ID from the DynastyProcess crosswalk; names are diagnostic only. Weekly team values are resolved to canonical NFL teams, including the explicit nflverse `LA` to app `LAR` normalization, and player detail reports all distinct teams in first-appearance order. Season rows are derived from weekly rows. The fixed half-PPR formula is 0.04/pass yard, 4/pass TD, -2/interception, 0.1/rush or receiving yard, 6/rush or receiving TD, 0.5/reception, -2/fumble lost, and 2/passing, rushing, or receiving two-point conversion, with no bonuses.
...
- 2026 odds: committed, user-supplied sportsbook CSV snapshot captured at `2026-08-31T01:23:28Z`. Import the supplied consensus columns directly as source `sportsbook_consensus`; do not recalculate them. Exact player full-name + NFL-team matching only, exact team-name matching for win totals, and no provider request. Blank consensus values remain missing; unmatched and position-invalid rows are reported and skipped.
- Morality index: committed, manually supplied 116-player CSV snapshot dated `2026-08-30`. Import integer 0–5 labels only by exact Sleeper player ID as source `user_supplied`; 5 is most favorable. The score is an opaque subjective user judgment, not calculated or independently verified by the application. Missing players remain unscored.
...
**M6.4 — FantasyPros projections.** Cache one approved all-position 2026 FantasyPros projection response and transactionally populate `player_projections` by exact FantasyPros ID. *Done when:* passing/rushing/receiving yards and TD forecasts are persisted with source/timestamp metadata; incomplete loads preserve prior data; no name guessing occurs; Overview shows all six fields; and Draft Day separates position-aware forecasts from grouped 2025 history. The `odds` table remains empty.
...
**M8.1 — Real season odds.** Import the approved, committed sportsbook-consensus snapshot for player futures and NFL-team wins. *Done when:* `load odds`, build, and rebuild transactionally produce the named/captured markets; unmatched and invalid-position subjects are reported without guessing; blank markets remain missing; player detail exposes explicit odds DTOs; and Draft Day displays a separate position-aware consensus card while preserving the FantasyPros card.

**M8.2 — User-supplied morality index.** Import the approved score-only, source-dated snapshot by exact Sleeper player ID and display its subjective 0–5 label in the Draft Day inspector, with 5 as the most favorable score. *Done when:* all 116 rows validate and load transactionally; unknown IDs, duplicates, and invalid scores reject the replacement; player detail exposes nullable score/provenance data; the inspector shows a same-sized box immediately left of FantasyPros tier; and the UI/docs identify it as a user-defined subjective label rather than an objective measure or app recommendation.

**M9 — Full usability audit.** Polish layout, focus and keyboard behavior, loading/error/empty states, table density and sticky context, chart legibility, stale-status UX, README guidance, naming, and dead code without adding major features. *Done when:* all three pages remain usable at common laptop widths; Draft Day clearly distinguishes live, syncing, polling-disabled, stale, browser-refresh-error, and unconfigured states; player selection/manual fallback and missing-data inspectors remain dependable; and the full repository checks plus a provider-free local smoke test pass.

---

## Deferred, on purpose

Single-binary build (Go serving `frontend/dist` via `embed.FS` — the Vite proxy setup makes this a drop-in later), replay mode, fantasy roster caching, and drafted-by-roster display. Revisit after the season, if ever.
